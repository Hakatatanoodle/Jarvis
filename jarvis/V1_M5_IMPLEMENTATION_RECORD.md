...
## Dogfooding fix #4 (2026-09-10): "read X and tell me Y" never dispatched fs.read at all — routing prompt had no filesystem examples

**Symptom**: "can you read the README.md file in this current repository
and tell me what you think about jarvis" got the reply "Sure, let me read
the `README.md` file for you. /fs.read README.md" — a fabricated
command string, not a real dispatched read (the actual read only
happened on a later, separate turn).

**Root cause — traced, not guessed**: the live transcript showed only 2
successful LLM calls for that whole turn. `conversation/api.py`'s
`_route()` makes exactly one routing call per turn, and each of
memory/state-change/capability extraction is its own separate call ONLY
if routing flags it possible — so 2 total calls (routing + the final
reply) is only possible if routing answered `False` to
`memory_candidate_possible`, `state_change_possible`, AND
`capability_invocation_possible` for the entire message, meaning it
classified "read the README and tell me what you think" as pure
ordinary conversation. Checked `_ROUTING_SYSTEM`'s actual prompt text:
section 4 (`capability_invocation_possible`) has detailed worked
examples for goal-touch and calendar-scheduling requests, but **zero**
for filesystem capabilities, and no example at all of a compound
"do X, then discuss Y" sentence — plausibly the trailing
conversational-sounding clause dominated the single-pass classification.
With `capability_executed`/`capability_relay` both `None`, the final
reply model (`_conversational_reply`) had nothing to ground an answer in
and improvised a promise plus a fabricated slash-command — the same
failure signature the 2026-08-31 fix already named ("hallucinated a fake
slash-command"), just reached via a different path (routing never even
tried extraction this time, vs. that fix's case of extraction running
with no data to relay).

`_route()` itself only ever logged on the failure/fallback path, so the
exact booleans for a *successful* call were invisible — this diagnosis
came from call-count inference, not a direct log line, which is itself
part of what's fixed below.

**Fix**:
- `conversation/api.py` `_ROUTING_SYSTEM`, section 4: added filesystem
  examples ('read the README.md file in this repo', 'what's in
  config.yaml') and explicit compound-request guidance — a request is
  still `true` here if *any part* of the message names a capability
  action, even when another part reads as conversational.
- `conversation/api.py` `_route()`: now logs the decided booleans on the
  success path too (previously silent), not just on fallback — pure
  observability, no behavior change.

**Same honest limitation as every other routing-prompt fix in this
record**: this can only pin that the prompt now covers the case, not
that a live model reliably classifies it correctly — that needs a real
transcript, same as the 2026-08-17/08-27/08-29 routing-prompt fixes
above it.

**Tests**: `tests/test_conversation_api.py` — one prompt-content test
(`test_routing_prompt_covers_filesystem_capability_and_compound_requests`,
same "prompt content only" pattern as this file's other routing-prompt
regressions) and one wiring test confirming `_route()` now actually logs
a successful decision's booleans. Full suite: `467 passed` before this
fix, `469 passed` after.

## Infra addition, decided with the person (2026-09-11): self-hosted LiteLLM proxy fronting Gemini/Groq, plus NVIDIA Nemotron and Mistral Codestral

**Context**: the person built and tested their own LiteLLM proxy
(docker-compose, `config.yaml`) specifically to attack the two weakest
links this whole free-tier-resilience track has been fighting: Groq's
8K TPM ceiling (2 keys round-robined ~doubles it) and Gemini's AQ-key
breakage (a second key/project might dodge the account-level 403 — not
guaranteed, since AQ-key issuance is account-level, not model-level;
flagged to the person to verify their two Gemini keys are genuinely
separate Google accounts, not two keys off the same broken one). Their
config also adds first-time access to NVIDIA NIM (Nemotron) and Mistral
(Codestral) — both wired in as new capacity, not replacements.

**Explicit scope, confirmed with the person before implementing**:
LiteLLM replaces the transport for Gemini and Groq ONLY. zai/CodeCraft/
OpenRouter stay exactly as direct providers, still wrapped by this same
module's own cooldown/retry/402-handling — LiteLLM is not a second
fallback-chain engine competing with this one, it's a transport swap
underneath 2 of the existing candidates.

**`infra/llm_router.py`**: new `LiteLLMProvider` — same OpenAI-compatible
shape as every other provider here, POSTs to
`http://localhost:4000/chat/completions` (the docker-compose port
mapping) with `Authorization: Bearer $LITELLM_MASTER_KEY`. `model` is
one of the person's own `model_name` aliases from their config.yaml
(`gemini-flash`, `groq-gptoss`, `groq-gpt-oss-20b`, `nvidia-nemotron`,
`codestral`), not a raw provider model id — LiteLLM resolves which
underlying key/model actually serves it. `GroqProvider`/`GeminiProvider`
left registered, unused, same precedent as `GitHubModelsProvider` after
its retirement.

**`config.yaml` (the person's LiteLLM proxy config, not this repo) —
one addition made and handed back**: their config only registered
`groq-gptoss` → `gpt-oss-120b` (dual-keyed). That's Jarvis's `hard`-route
model; routing ordinary `chat` traffic through it instead of the
smaller/faster 20b would work but is slower per call and burns the same
TPM budget faster for no benefit on simple messages. Added a
`groq-gpt-oss-20b` model group, same dual-key pattern as the existing
`groq-gptoss` entry, so `chat` keeps its own cheap/fast tier distinct
from `hard`'s bigger model — this was the specific judgment call the
person deferred to me on.

**Routes rebuilt** (`config/default.yaml`):
- `chat`: zai `glm-4.7-flash` (unchanged, direct) → litellm
  `groq-gpt-oss-20b` → litellm `gemini-flash`.
- `hard`: codecraft `deepseek-v4-flash-0731` (unchanged, direct) →
  litellm `nvidia-nemotron` (new capacity, reasoning-oriented, ahead of
  the TPM-capped Groq rung) → litellm `groq-gptoss` (120b) → litellm
  `gemini-flash`.
- `coding`: codecraft `deepseek-v4-flash-0731` (unchanged, direct) →
  litellm `codestral` (Mistral's actual dedicated code model — a better
  fit than the generic qwen-coder that used to lead here) → openrouter
  `qwen/qwen3-coder:free` (unchanged, direct, kept as further fallback)
  → litellm `gemini-flash`.
- Emergency pool unchanged (`openrouter/free`).

**`.env.example`**: `GROQ_API_KEY`/`GEMINI_API_KEY` removed — those keys
now live in the LiteLLM proxy's own `.env` (its docker-compose
`env_file`), not Jarvis's, since Jarvis no longer calls either directly.
Added `LITELLM_MASTER_KEY`, which must be the exact same value as the
proxy's own `general_settings.master_key` / its container's
`LITELLM_MASTER_KEY` — this is Jarvis authenticating to the proxy, not a
per-provider key.

**New operational dependency, flagged to the person**: `python3 cli.py`
now requires the `litellm-proxy` container running on `localhost:4000`
before any of chat/hard/coding's Gemini/Groq/Nemotron/Codestral rungs
can succeed — if the container isn't up, 4 of the current candidates
across all three routes fail at once with a connection error (correctly
classified `RETRYABLE` by the existing error-classification logic, same
as any other network failure, so the router still falls through to
zai/codecraft/openrouter correctly — but worth knowing this is now a
single point of failure in front of more than one provider).

**Not verified against a live LiteLLM proxy** — same standing caveat as
every other provider added this week: verified against LiteLLM's
documented OpenAI-compatible proxy shape and the person's own tested
`config.yaml`, not against a live call from this sandbox (no network
route to `localhost:4000` from here, and it isn't running in this
sandbox anyway).

**Tests**: `tests/test_llm_router.py` — every chat/hard/coding
route-order test touching Groq/Gemini directly rewritten around
`LiteLLMProvider` (`test_chat_zai_fails_falls_back_to_litellm_groq_20b`,
`test_hard_codecraft_fails_falls_back_to_litellm_nemotron`, `test_coding
_codecraft_402_quota_exhausted_falls_back_to_codestral`, etc.), the
vision-capability test moved to `gemini-flash` as the sole vision-tagged
hard-route rung, plus 3 new `LiteLLMProvider`-body tests (truncation-
retry, master-key/model-alias sent correctly, not-configured).
`GroqProvider`/`GeminiProvider` keep their own existing provider-body
tests since the classes are untouched, just unused in routes now. Full
suite: `469 passed` before this change, `474 passed` after.

## Rename, decided with the person (2026-09-13): Jarvis → Nika

**Scope decided up front, not applied blindly**: the person is moving off
"Jarvis" as their own project's identity. Renamed everywhere the name is
either user/model-facing (what the assistant calls itself, what the CLI
prints, log output) or plain descriptive prose in *currently-active*
source. Explicitly did NOT touch:
- **Historical planning/record docs** (`V1_M1`–`V1_M6_IMPLEMENTATION_
  RECORD.md`, `MERGE_NOTES_M5_M6.md`, `*_HANDOVER_PROMPT.md`,
  `GROUNDED_CONVERSATION_IMPLEMENTATION_RECORD.md`, `ARCHITECTURE_
  ISSUES.md`, `V1_PLANNING_INPUT.md`, `V1_M1_IMPLEMENTATION_PLAN.md`,
  everything under `docs/rfcs/`) — these are dated record of what was
  built and why, under the name it had at the time; rewriting them would
  be revisionist. Live source citing one of these by filename (e.g.
  `HOW_JARVIS_THINKS.md`, `Jarvis_Architecture_Review_v1.md`,
  `Jarvis_V0_Testing_Review`) keeps the real filename too.
- **Postgres identifiers** — `storage.postgres.database`/`.user` defaults
  (`"jarvis"`), `docker-compose.yml`'s `POSTGRES_USER`/`POSTGRES_DB`/
  volume name, and `mission/api.py`'s advisory-lock key were evaluated
  individually. The advisory-lock key (`"jarvis.mission.single_active_
  invariant"`) was renamed — it's a same-transaction runtime lock
  namespace, never persisted, zero migration risk. The Postgres defaults
  were **not** renamed — the person's real, already-running database is
  literally named `jarvis`/owned by role `jarvis`; changing the default
  here wouldn't rename their live DB, it would just make a fresh install
  point at a database that doesn't exist. Left alone deliberately, not
  overlooked.
- **The `jarvis/` folder/package name itself** and the person's on-disk
  path (`~/Projects/Jarvis/jarvis`) — cosmetic only (no Python import
  depends on the directory name), and renaming it would mean the person
  moving/renaming their actual checkout for zero functional benefit.
  Source comments illustrating real path examples (`~/Projects/Jarvis/
  jarvis-M6`, `fs.read`'s own worked example of resolving `"the jarvis
  project" -> path_ref "jarvis/README.md"`) keep saying "jarvis" because
  that's genuinely still the folder's name.
- **Illustrative goal-title examples** ("Build Jarvis and complete an
  ironman race") in `reasoning/api.py`'s docstrings — these mirror the
  person's actual real, already-stored goal title from live dogfooding,
  not a placeholder; renaming the comment would make it inaccurate
  relative to what's really in their database.

**What did change** (functional, not just cosmetic, where noted):
- `infra/logging.py`: `_ROOT_NAME` `"jarvis"` → `"nika"` — every log
  line's prefix.
- `cli.py`: the printed banner, `/help` text, and non-historical
  docstrings/comments.
- Every system-prompt string that has the assistant self-identify —
  `conversation/api.py` (`_ROUTING_SYSTEM`, `_CONVERSATIONAL_SYSTEM`,
  `_GROUNDED_SYSTEM`), `reasoning/api.py`, `insight/api.py` — plus all
  surrounding prose/comments in those files and `capability_invocation/
  extraction.py`, `capability_invocation/dispatch.py`, `state_change/
  extraction.py`, `memory/extraction.py`, `memory/candidate_policy.py`,
  `capabilities/primitives/goal_ops.py`.
- `conversation/api.py`'s `_GREETING_FILLER` set — functional: this gates
  the fast-path trivial-greeting detector; `"nika"` added (kept
  `"jarvis"` too — harmless if anyone types the old name out of habit)
  so "hey Nika" still hits the fast path instead of a full routing call.
- `conversation/api.py`'s recent-conversation-history builder — the
  `"Jarvis: {response}"` transcript label fed back into prompts as
  context is now `"Nika: {response}"`, so the model doesn't see its own
  system prompt call it Nika while its own prior turns in context are
  labeled Jarvis.
- `README.md` title/description (current living doc, not a historical
  record) and `config/default.yaml`'s top comment + two prose comments
  (not its Postgres defaults, per above).

**Tests**: existing suite caught every functionally-load-bearing string
change on its own — 3 pre-existing tests asserted exact lowered prompt
substrings (`"jarvis's name"`, `"jarvis itself"`) or the exact logger
name (`"jarvis.goals"`) and failed immediately after the rename, updated
to match. No new tests needed; this wasn't new behavior, just an
identity string, and the suite's existing string-assertion style already
covers it. Full suite: `474 passed` before this rename, `474 passed`
after (3 updated, 0 net new, 0 broken).
