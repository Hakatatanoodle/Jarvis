...
a bare follow-up answering Nika's own clarifying question carries no
...
_EXTRACTION_SYSTEM = (
    ...
    "means Nika will have to ask the user to clarify.\n\n"
    ...
    "not an error; it lets Nika check every connected account for a "
    ...
    "Capabilities V2 notes: for web.search, put the concise search "
    "query in extra.query. For reminders.create, extra.reminder_text is "
    "what to remind about and extra.due_at is a full ISO 8601 datetime "
    "with the user's UTC offset, resolved from their LOCAL time given "
    "below exactly as described for start/end above (\"in 20 minutes\" = "
    "current local time + 20 minutes; \"tomorrow at 9\" = tomorrow's "
    "local date at 09:00). For reminders.cancel, extra.cancel_target is "
    "a word or phrase from the reminder's text. Any of these missing "
    "means Nika will ask the user, so only leave one null if truly not "
    "given.\n\n"
    "Continuing a clarifying question: you are also shown recent "
    "conversation from this session. If Nika's most recent message "
...
def _build_prompt(
    ...
    recent_block = (
        f"Recent conversation this session (check whether Nika's last "
