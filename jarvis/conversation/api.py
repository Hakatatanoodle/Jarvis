...
PROJECT.md's "Nika should always be able to explain..."), but never
...
_GREETING_FILLER = {"good", "you", "jarvis", "nika", "there"}
...
_ROUTING_SYSTEM = (
    ...
    "1. needs_reasoning: does this require Nika's goal/mission/memory-"
    ...
    "Nika in the abstract, even though 'what are the statuses I can "
    ...
    "fact about them, a standing instruction for how Nika should "
    ...
    "request for Nika to do something, small talk with nothing "
    ...
    "Nika asking a clarifying question about a Goal/Mission command "
    ...
    "to Nika's own pending question about one.\n\n"
    ...
    "what you think about it' is STILL true here (fs.read), even "
    "though the second half is a conversational-sounding ask; say true "
    "whenever ANY part of the message plausibly names a capability "
    "action, not only when the whole message is about it. A request "
    ...
    "session ends with Nika asking a clarifying question about which "
...
_CONVERSATIONAL_SYSTEM = (
    "You are Nika, a personal AI operating system, responding in ordinary "
    ...
    "(e.g. \"user's name\") — never a memory about Nika's own name or "
    "identity (e.g. \"Nika's name\"), never something inferred from "
...
# turns still get included either way — it just means Nika describes
...
def _format_recent_context(turns: list[ConversationTurn]) -> str:
    ...
    return "\n".join(f"User: {t.user_text}\nNika: {t.response_text}" for t in turns)
...
async def _apply_state_change_policy(
    ...
    if decision.outcome is StateChangePolicyOutcome.ASK_CONFIRMATION:
        ...
        relay = (
            ...
            f"Nika proceeds — {decision.reason}"
        )
...
async def _route(user_text: str, recent_context: Optional[str] = None) -> RouteDecision:
    ...
    # follow-up answer to Nika's own clarifying question ("a Project",
    ...
    if (
        ...
        # Dogfooding fix (2026-09-10): this call previously logged nothing
        # at all on success — only the failure/fallback path below logged
        # anything, which made a real routing miss (a capability
        # instruction embedded in a longer message, classified false)
        # invisible except by inference from which LLM calls happened
        # downstream. Cheap, no behavior change, makes the next version
        # of this exact bug directly diagnosable instead of inferred.
        log.info(
            f"Routing decision: needs_reasoning={decision.needs_reasoning} "
            f"memory_candidate_possible={decision.memory_candidate_possible} "
            f"state_change_possible={decision.state_change_possible} "
            f"capability_invocation_possible={decision.capability_invocation_possible}"
        )
        return decision
...
async def _conversational_reply(
    ...
    if relay_note:
        ...
        # "Nika can't complete yet", which combined with
        ...
        prompt += (
            f"Nika is actively handling a requested command from the "
            f"user right now and it isn't finished yet — this is a "
            f"normal in-progress step, not something Nika lacks the "
...
_GROUNDED_SYSTEM = (
    "You are Nika, answering a question that needs real grounding in "
...
async def _persist_grounded_decision(user_text: str, response_text: str, context_items, mission) -> None:
    ...
    PROJECT.md: 'Nika should always be able to explain: why it made a
