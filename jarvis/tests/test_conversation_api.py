...
# --- regression: 2026-09-10 dogfooding -----------------------------------
# "can you read the README.md file in this current repository and tell me
# what you think about jarvis" never dispatched fs.read at all — call-count
# evidence in the live transcript showed only 2 successful LLM calls that
# turn (routing, then the final reply), meaning no third call for
# capability extraction ever happened, meaning routing itself must have
# answered capability_invocation_possible=False for the whole message.
# _ROUTING_SYSTEM had detailed worked examples for goals and calendar in
# that section, but zero for filesystem capabilities, and no example at
# all of a compound "do X, then discuss Y" sentence — the trailing
# conversational-sounding clause plausibly dominated the classification.
# With no capability_executed/capability_relay to ground it, the final
# reply model had nothing to work with and improvised "Sure, let me read
# the file for you. /fs.read README.md" — a fabricated command string,
# the same failure signature as the 2026-08-31 fix's "hallucinated a fake
# slash-command" (that fix only helps once dispatch already ran with no
# data; this message never reached dispatch at all). Same honest
# limitation as every other routing-prompt test in this file: this can
# only assert the prompt now covers the case, not that a given model
# reliably classifies it correctly.

def test_routing_prompt_covers_filesystem_capability_and_compound_requests():
    lowered = _ROUTING_SYSTEM.lower()
    assert "read the readme.md file in this repo" in lowered
    assert "what's in config.yaml" in lowered
    assert "still true here (fs.read)" in lowered
    assert "any part of the message plausibly names a capability action" in lowered


async def test_route_logs_decision_on_success_not_just_on_fallback():
    # Dogfooding fix, same date: _route() logged nothing at all on success
    # — only the failure/fallback path did — so a real routing miss like
    # the one above was invisible except by inferring it from which LLM
    # calls happened downstream. This pins that a successful decision is
    # now actually observable.
    raw = {
        "needs_reasoning": False, "memory_candidate_possible": False,
        "state_change_possible": False, "capability_invocation_possible": True,
    }
    with patch("conversation.api.complete_json", new=AsyncMock(return_value=raw)), \
         patch("conversation.api.log") as mock_log:
        from conversation.api import _route
        decision = await _route("read the README.md file and tell me what you think")
    assert decision.capability_invocation_possible is True
    logged = " ".join(str(c) for c in mock_log.info.call_args_list)
    assert "capability_invocation_possible=True" in logged
...
# this covers ALREADY-CONTAMINATED data (a "nika's name" row that
...
# contaminated "nika's name" row was forgotten, Nika kept insisting
