...
def _clean_router_state(monkeypatch):
    ...
    for var in ("GROQ_API_KEY", "GEMINI_API_KEY", "OPENROUTER_API_KEY",
                "ZAI_API_KEY", "CODECRAFT_API_KEY", "GITHUB_TOKEN", "LITELLM_MASTER_KEY"):
...
# --------------------------------------------------------------------------
# Chat route — reworked 2026-09-11: the person stood up a self-hosted
# LiteLLM proxy (docker-compose) fronting Gemini and Groq with 2 keys
# each for more TPM headroom, plus first-time NVIDIA/Mistral access. The
# direct groq/gemini route entries are replaced with litellm ones; zai
# is untouched (still the best genuinely-unlimited-free leader). See
# V1_M5_IMPLEMENTATION_RECORD.md's 2026-09-11 entry.
# --------------------------------------------------------------------------

async def test_chat_primary_zai_succeeds():
    with patch.object(ZaiProvider, "call", new=AsyncMock(return_value="hi")) as zai, \
         patch.object(LiteLLMProvider, "call", new=AsyncMock()) as ll:
...
async def test_chat_zai_fails_falls_back_to_litellm_groq_20b():
    with patch.object(ZaiProvider, "call", new=AsyncMock(side_effect=_status_error(429))), \
         patch.object(LiteLLMProvider, "call", new=AsyncMock(return_value="fallback ok")) as ll:
        result = await complete("hi", task_type="chat")
        assert result == "fallback ok"
        assert ll.call_args.args[0] == "groq-gpt-oss-20b"


async def test_chat_zai_and_litellm_groq_fail_falls_back_to_litellm_gemini():
    async def ll_side_effect(model, *a, **kw):
        if model == "groq-gpt-oss-20b":
            raise _status_error(500)
        return "gemini ok"

    with patch.object(ZaiProvider, "call", new=AsyncMock(side_effect=_status_error(500))), \
         patch.object(LiteLLMProvider, "call", new=AsyncMock(side_effect=ll_side_effect)):
        result = await complete("hi", task_type="chat")
...
async def test_chat_all_fail_raises():
    with patch.object(ZaiProvider, "call", new=AsyncMock(side_effect=_status_error(500))), \
         patch.object(LiteLLMProvider, "call", new=AsyncMock(side_effect=_status_error(500))), \
...
# --------------------------------------------------------------------------
# Hard route — reworked 2026-09-11 (see chat route's comment above for the
# LiteLLM context): CodeCraft still leads, then NVIDIA Nemotron (new
# capacity via the proxy), then LiteLLM-fronted Groq 120b / Gemini.
# --------------------------------------------------------------------------
...
async def test_hard_codecraft_fails_falls_back_to_litellm_nemotron():
    with patch.object(CodeCraftProvider, "call", new=AsyncMock(side_effect=_status_error(429))), \
         patch.object(LiteLLMProvider, "call", new=AsyncMock(return_value="nemotron ok")) as ll:
        result = await complete("think", task_type="hard")
        assert result == "nemotron ok"
        assert ll.call_args.args[0] == "nvidia-nemotron"


async def test_hard_codecraft_and_nemotron_fail_falls_back_to_litellm_groq_120b():
    async def ll_side_effect(model, *a, **kw):
    ...
    with patch.object(CodeCraftProvider, "call", new=AsyncMock(side_effect=_status_error(500))), \
         patch.object(LiteLLMProvider, "call", new=AsyncMock(side_effect=ll_side_effect)):
        result = await complete("think", task_type="hard")
        assert result == "groq 120b ok"


async def test_hard_all_litellm_rungs_fail_falls_back_to_litellm_gemini():
    async def ll_side_effect(model, *a, **kw):
...
# --------------------------------------------------------------------------
# Coding route — reworked 2026-09-11: CodeCraft still leads, then Mistral
# Codestral (new — a model actually built for code, via the proxy) ahead
# of the existing OpenRouter qwen-coder, then LiteLLM-fronted Gemini.
# --------------------------------------------------------------------------
...
async def test_coding_codecraft_402_quota_exhausted_falls_back_to_codestral():
    ...
    with patch.object(CodeCraftProvider, "call", new=AsyncMock(side_effect=_status_error(402))), \
         patch.object(LiteLLMProvider, "call", new=AsyncMock(return_value="codestral ok")) as ll:
        result = await complete("write a function", task_type="coding")
        assert result == "codestral ok"
        assert ll.call_args.args[0] == "codestral"


async def test_coding_codecraft_and_codestral_fail_falls_back_to_qwen():
    with patch.object(CodeCraftProvider, "call", new=AsyncMock(side_effect=_status_error(500))), \
         patch.object(LiteLLMProvider, "call", new=AsyncMock(side_effect=_status_error(500))), \
...
async def test_coding_all_but_litellm_gemini_fail_falls_back_to_it():
    async def ll_side_effect(model, *a, **kw):
        if model == "gemini-flash":
            return "gemini code ok"
        raise _status_error(500)
...
async def test_not_configured_skips_straight_to_next_candidate_no_retry_sleep():
    with patch.object(ZaiProvider, "call", new=AsyncMock(side_effect=ProviderNotConfiguredError("no key"))), \
         patch.object(LiteLLMProvider, "call", new=AsyncMock(return_value="ok")) as ll, \
...
async def test_failed_candidate_enters_cooldown_and_is_skipped_next_call():
    with patch.object(ZaiProvider, "call", new=AsyncMock(side_effect=_status_error(500))), \
         patch.object(LiteLLMProvider, "call", new=AsyncMock(return_value="ok")):
        await complete("hi", task_type="chat")
    ...
    with patch.object(ZaiProvider, "call", new=AsyncMock()) as zai, \
         patch.object(LiteLLMProvider, "call", new=AsyncMock(return_value="ok again")):
        result = await complete("hi", task_type="chat")
...
async def test_capability_requirement_skips_incapable_candidate():
    # None of chat's candidates (zai/litellm-groq-20b/litellm-gemini, nor
    # the text/json-only emergency rung) are tagged for vision; requiring
    # it must filter the whole chain to empty and raise immediately
...
async def test_no_candidate_satisfies_capability_raises_immediately():
    with patch.object(LiteLLMProvider, "call", new=AsyncMock()) as ll:
        with pytest.raises(LLMUnavailableError):
            await complete("hi", task_type="chat", capabilities={"code_execution"})
        ll.assert_not_called()


async def test_hard_route_vision_capability_selects_litellm_gemini():
    # Neither CodeCraft's, Nemotron's, nor Groq 120b's hard-route models
    # are tagged for vision — only litellm/gemini-flash is, last in the
    # chain. Requiring vision should skip straight past the first three.
    async def ll_side_effect(model, *a, **kw):
        if model == "gemini-flash":
            return "ok"
        raise AssertionError(f"should not have been called for {model}")

    with patch.object(CodeCraftProvider, "call", new=AsyncMock()) as cc, \
         patch.object(LiteLLMProvider, "call", new=AsyncMock(side_effect=ll_side_effect)) as ll:
        result = await complete("describe this image", task_type="hard", capabilities={"vision"})
        assert result == "ok"
        ll.assert_called_once_with("gemini-flash", ANY, ANY, ANY)
...
# --- 2026-09-11: LiteLLM proxy (self-hosted, fronts Gemini/Groq/NVIDIA/
# Mistral) — same real-body pattern as every other provider above.

async def test_litellm_retries_once_on_finish_reason_length():
    truncated = _fake_response({"choices": [{"message": {"content": "cut off mid"}, "finish_reason": "length"}]})
    complete_reply = _fake_response({"choices": [{"message": {"content": "a full sentence now."}, "finish_reason": "stop"}]})
    post_mock = AsyncMock(side_effect=[truncated, complete_reply])
    with patch.object(httpx.AsyncClient, "post", new=post_mock):
        provider = LiteLLMProvider()
        result = await provider.call("gemini-flash", "sys", "prompt", max_tokens=100)

    assert result == "a full sentence now."
    assert post_mock.call_count == 2


async def test_litellm_sends_master_key_and_model_alias(monkeypatch):
    monkeypatch.setenv("LITELLM_MASTER_KEY", "sk-test-master")
    response = _fake_response({"choices": [{"message": {"content": "ok"}, "finish_reason": "stop"}]})
    post_mock = AsyncMock(return_value=response)
    with patch.object(httpx.AsyncClient, "post", new=post_mock):
        provider = LiteLLMProvider()
        await provider.call("groq-gpt-oss-20b", "sys", "prompt", max_tokens=100)

    call_kwargs = post_mock.call_args.kwargs
    assert call_kwargs["headers"]["Authorization"] == "Bearer sk-test-master"
    assert call_kwargs["json"]["model"] == "groq-gpt-oss-20b"
    assert post_mock.call_args.args[0] == "http://localhost:4000/chat/completions"


async def test_litellm_not_configured_without_master_key(monkeypatch):
    monkeypatch.delenv("LITELLM_MASTER_KEY", raising=False)
    provider = LiteLLMProvider()
    with pytest.raises(ProviderNotConfiguredError):
        await provider.call("gemini-flash", "sys", "prompt", max_tokens=100)
