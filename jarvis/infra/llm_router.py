...
# --------------------------------------------------------------------------
# Provider abstraction (§3). Nika's core logic never sees Gemini/Groq/
...
class LiteLLMProvider(LLMProvider):
    """Self-hosted LiteLLM proxy (docker-compose, localhost:4000), added
    2026-09-11 — see V1_M5_IMPLEMENTATION_RECORD.md's entry that day.
    NOT a new inference provider — it's a routing layer the person built
    themselves in front of Gemini and Groq specifically (their own
    config.yaml holds 2 keys per model, letting LiteLLM round-robin
    between them), plus first-time access to NVIDIA NIM and Mistral
    Codestral. zai/CodeCraft/OpenRouter are untouched, direct, and still
    wrapped by THIS module's own cooldown/retry/402-handling exactly as
    before — LiteLLM only replaces the transport for the two providers
    it fronts, it doesn't replace this router's own fallback chain.
    `model` here is one of the person's own `model_name` aliases in
    their config.yaml (e.g. "gemini-flash", "groq-gptoss",
    "groq-gpt-oss-20b", "nvidia-nemotron", "codestral"), not a raw
    provider model id — LiteLLM resolves which underlying key/model
    ...
    async def call(self, model: str, system: str, prompt: str, max_tokens: int) -> str:
        ...
        if not key:
            raise ProviderNotConfiguredError("LITELLM_MASTER_KEY not set")
        ...
        if choice.get("finish_reason") == "length" and _truncation_retry_allowed(max_tokens):
            ...
            if retried_content:
                content = retried_content
        if not content:
            raise LLMUnavailableError(f"litellm/{model} returned empty/null content")
        return content
